-- Let the ranked search RPC filter item condition in SQL.
-- Client-side filtering after LIMIT could hide valid results.

DROP FUNCTION IF EXISTS public.search_items(text, text, integer, integer);
...
REVOKE ALL ON FUNCTION public.search_items(text, text, text, integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.search_items(text, text, text, integer, integer) TO authenticated;
